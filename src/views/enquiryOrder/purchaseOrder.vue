        ...
        <el-col :span="24">
          <el-form :inline="true" :model="formQuery" class="demo-form-inline">
            ...
            <el-form-item>
              ...
              <el-button :loading="succSync" type="primary" icon="el-icon-sort" @click="manualSync">{{ sync }}</el-button>
              <el-button type="primary" icon="el-icon-upload2" @click="sendStatus()">手动推送</el-button>
              <el-button type="primary" icon="el-icon-error" @click="cancelSendStatus()">取消推送</el-button>
            </el-form-item>
        ...
        <template>
          ...
          >
            ...
            <el-table-column
              ...
              show-overflow-tooltip
            />
            <el-table-column prop="sync" label="推送状态" width="80" align="center">
              <template slot-scope="scope">
                <!-- <span :class="{green:scope.row.sync== true,red:scope.row.sync==false}">
                    {{scope.row.sync==false ? '未推送' : '已推送'}}
                  </span> -->
                <el-tag
                  :type="scope.row.sync ? 'success' : 'danger'"
                  size="mini"
                  disable-transitions>{{ scope.row.sync ? '已推送': '未推送' }}</el-tag>
              </template>
            </el-table-column>
            <el-table-column
              prop="f_103"
            ...
            <el-table-column
              ...
              show-overflow-tooltip
            />
            <el-table-column label="供应商回复交期" width="120" align="center" show-overflow-tooltip fixed="right">
              <template slot-scope="scope">
                <span>
                  {{ scope.row.replyDate!=null?getTime(scope.row.replyDate):'' }}
                </span>
              </template>
            </el-table-column>
...
<script>
import { getOrderList, sendOrder, cancelSend, manual } from '@/api/purchaseOrderK3'
