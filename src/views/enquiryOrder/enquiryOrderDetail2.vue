  ...
  <div>
          ...
          <el-row type="flex" class="row-bg">
            <el-col :span="24">
              ...
              <el-form-item>
                ...
                <el-button type="success" size="mini" @click="getExcel">导出清单</el-button>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
    ...
    <template>
      <el-table
        ...
        <el-table-column prop="bsQty" label="数量" show-overflow-tooltip width="100">
        </el-table-column>
...
<script>
import { getDetailList_2,getQuoteMateList } from "@/api/enquiryOrder";
...
export default {
  ...
  methods: {
    getData() {
      ...
      this.loading = true;
      //获取BOM-询价列表
      getDetailList_2(filter).then(response => {
        this.loading = false;
        if (response.result) {          
    ...
    goBack() {
      this.$router.push({path:'/enquiry/enquiryOrderDetail',query:{bsOrderId:this.bsOrderId}});
    },
    getExcel() {
      window.location.href =
      process.env.BASE_API + "/enquiryOrderDetail/getDetailExcel?bsOrderId=" + this.bsOrderId;
    },
    //重置
    resetFrom(){
