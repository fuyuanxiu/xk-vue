  ...
  <div>
      ...
      <el-row>
        <el-col :span="6" style="width:286px">
          <el-button-group style="margin-left:18px;margin-bottom: 10px;">
            <el-button v-if="permit.ADD" type="primary" icon="el-icon-plus" @click="showAddCategory()">新增工段</el-button> 
            <el-button v-if="permit.EDIT" type="primary" icon="el-icon-edit" @click="showEditCategory()">编辑工段</el-button>             
        ...
        <el-col :span="16" style="margin-left:10px">
          <el-form :inline="true" :model="queryParams" class="demo-form-inline">
            ...
            <el-form-item>
              ...
              <el-button v-if="permit.ADD" type="primary" icon="el-icon-plus" @click="showAddData()">新增工序</el-button>
            </el-form-item>
          </el-form>
        </el-col>     
      </el-row>
    ...
    <el-container style="height: 500px;">
      ...
      <el-aside class="tree">
        ...
        @current-change="handleSelectionChange">  
          ...
          <el-table-column  width="230" prop="bsName" label="工段名称">
          </el-table-column>
        </el-table>
      ...
      <el-main>
        <el-table :data="iTable"> 
          <el-table-column prop="bsCateName" label="工段名称" show-overflow-tooltip width="150">
    ...
    <el-dialog title="新增或编辑类别" :visible.sync="categoryVisible" width="35%">
      <el-form :inline="true" :model="categoryForm" ref="ruleForm1">
        ...
        <el-form-item label="工段名称" prop="bsName">
    ...
    <!-- 新增工序 -->
    <el-dialog title="新增工序" :visible.sync="dialog.dataVisible" width="500px">
      <el-form
      :model="dataForm"
    ...
    <!-- 编辑工序 -->
    <el-dialog title="编辑工序" :visible.sync="dialogEdit.dataVisible" width="500px">
      <el-form
      :model="dataFormEdit"
      ...
      label-width="110px">
        ...
        <el-form-item label="工序编号" prop="bsCode">
...
<script>
import {getCategory,addCategory,editCategory,delCategory,getProcess,addProcess,editProcess,delProcess,banProcess} from '@/api/process'
  ...
  methods:{ 
    ...
    showEditData(row){       
      ...
      const newRow = {
        ...
        bsCateName:(row.processCategory) ? row.processCategory.bsName : '',
        bsCode:row.bsCode,
        bsName:row.bsName,
    ...
    addBtn(){    
      ...
      this.$refs['dialog.keyruleForm'].validate((valid) => {               
        if (valid) {
          addProcess(filter).then(response => {                     
            if(response.result) {
              this.$message.success(response.msg);                       
    ...
    editBtn(){
      ...
      this.$refs['dialogEdit.keyruleForm'].validate((valid) => {
        if (valid) {
          editProcess(filter).then(response => {                                   
            if(response.result) {                                
    ...
    doBan(isBan, row){
      ...
      this.$confirm(contents, '提示', {
        ...
        //禁用
        banProcess(row.id, isBan).then(response=>{
          if(response.result){
            this.$message({
                message:response.msg,
                type:'success'
