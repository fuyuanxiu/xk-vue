<template>
  <div style="float: right;margin-left: 15px;">
    ...
    <el-button :loading="loading" type="primary" @click="handleUpload">K3代码导入</el-button>
  </div>
</template>

<script>
import XLSX from 'xlsx'
import {uplaodFile} from '@/api/costMate'
  ...
  methods: {
    ...
    handleClick(e) {
      ...
      this.$prompt('请输入开始读取的行数', '提示', {
          ...
          formData.append('bomParamId', this.bomParamId);
          formData.append('fileId', this.fileId);
          formData.append('bomK3CodeCol', this.bomK3CodeCol);
        ...
        //end
          if (!rawFile) return
          //this.upload(rawFile)//展示页面数据 直接后后台读取数据
        })
    },
    upload(rawFile) {
