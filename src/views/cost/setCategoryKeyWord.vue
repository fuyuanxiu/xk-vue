...
<!--   表格内容区 -->
      ...
      <el-table :data="keywordTable"> 
        <el-table-column prop="bsCateName" label="类别名称" show-overflow-tooltip width="200">
        </el-table-column>      
        ...
        <el-table-column prop="address" label="操作"  width="180">
            <template slot-scope="scope">
              <el-button v-if="permit.EDIT" :disabled="scope.row.isChecked?true:false" type="primary" icon="el-icon-edit" size="mini" circle @click="showEditKeyword(scope.row)"></el-button>
              <messagebox v-if="permit.DELETE" :disabled="scope.row.isChecked?true:false"
                @callConfirm="delKeywordlist(scope.row)"
                title="提示"
                contents="此操作将永久删除该行, 是否继续?"
                confirmTitle="确认删除"
                ></messagebox> 
                  <el-button v-if="permit.CHECK" :disabled="scope.row.isChecked?true:false"  type="primary" icon="el-icon-success" size="mini" circle title="审核" @click="check(scope.row.id)"></el-button>
                <el-button v-if="permit.UNCHECK" :disabled="!scope.row.isChecked?true:false"  type="success" icon="el-icon-error" size="mini" circle title="反审核" @click="uncheck(scope.row.id)"></el-button>              
            </template>
        </el-table-column>
      </el-table>
    </el-main>
...
<!--  新增物料 -->
        ...
        >
          ...
          <div
            ...
            class="dialog-footer"
          >
            <el-button class="el-icon-close" @click="keywordsDialogVisible = false">取 消</el-button>
            <el-button
              class="el-icon-check"
              type="primary"
              @click="matchFormOk"
        ...
        >
          ...
          >
           <el-form-item label="类别ID" style="" prop="bsCateId"  label-position="left" >
                <el-input v-model="keywordForm.bsCateId" style="width:218px;" :disabled="true"></el-input>
            </el-form-item>             
            <el-form-item label="分类名称"  prop="bsCateName"  >
                 <el-input v-model="keywordForm.bsCateName" style="width:218px;" :disabled="true"></el-input>                 
            </el-form-item>
             <el-form-item label="关键字名称" prop="bsName">
                <el-input v-model="keywordForm.bsName" style="width:218px;"></el-input>
            </el-form-item>       
             <el-form-item label="填写备注" prop="bsRemark">
                <el-input v-model="keywordForm.bsRemark" style="width:218px;">
                </el-input>
            </el-form-item>   
          ...
          <div
            ...
            class="dialog-footer"
          >
            <el-button class="el-icon-close" @click="dialog.keywordVisible = false">取 消</el-button>
            <el-button  class="el-icon-check" type="primary" @click="addKeywordsBtn">确 定</el-button>
          </div>
        </el-dialog> 
     <div class="block">
      <el-pagination
...
<script>
import {getCategorylist,addCategoryKey,editCategoryKey,delCategoryKey,addKeywords2,delKeywords2,getlistKeywords2,editKeywords2,modifyCheck,reverseCheck} 
from '@/api/keywordsCategory'
...
export default {
      ...
      dialog: {        
        ...
        keyruleForm: {        
          ...
          bsCateName: [
           ...
           bsName: [
            { required: true, message: "关键字名称", trigger: "blur" }
          ],    
  ...
  methods:{ 
    ...
    let filter=Object.assign({},{cateId:this.selection.id, keyword:this.queryParams.keyword,rows:this.queryParams.rows,page:this.queryParams.page})      
        getlistKeywords2(filter).then(response => {
            if (!response.data) { 
            reject('error')
          }            
                ...
                this.$refs['ruleForm1'].validate((valid) => {
                    if (valid) {
                       /* this.dialog.loading = true;*/
                        editCategoryKey(filter).then(response => {                                            
                        if(response.result) {                                
    ...
    doDelete(){        
        ...
        delCategoryKey(id).then(response=>{
            if(response.result){
                this.$message({
    ...
    if(typeof(this.keywordForm.id)!=undefined && typeof(this.keywordForm.id)=="number") {             
            this.$refs['dialog.keyruleForm'].validate((valid) => {
                if (valid) {
                   /* this.dialog.loading = true;*/
                    editKeywords2(filter).then(response => {                                   
                    if(response.result) {                                
             ...
             this.$refs['dialog.keyruleForm'].validate((valid) => {               
                if (valid) {
                    addKeywords2(filter).then(response => {                     
                        if(response.result) {                       
                            this.dialog.keywordVisible=false;
                            this.getKeywordList();
                        }else {
