...
<template>
  <div element-loading-text="数据加载中，请稍等">
    ...
    <el-table :data="iTable">
      ...
      <el-table-column prop="bsMachine" label="机型" show-overflow-tooltip width="150">
        <template scope="scope">
          <span>{{ scope.row.marketReport ? scope.row.marketReport.bsMachine : "" }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="bsProcess" label="项目" width="150" show-overflow-tooltip>
        <template scope="scope">
      ...
      <el-table-column prop="bsUnit" label="单位" width="150" show-overflow-tooltip>
      </el-table-column>
      <el-table-column prop="price3" label="含税单价" width="150" show-overflow-tooltip>
        <template scope="scope">
          <span>{{ returnFloat_2(scope.row.price3) }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="price3Total" label="含税金额" width="150" show-overflow-tooltip>
        <template scope="scope">
          <span>{{ returnFloat_2(scope.row.price3Total) }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="bsRemark" label="备注" minWidth="150" show-overflow-tooltip>
      </el-table-column>
    </el-table>
    <div class="block">
      <el-pagination
...
export default {
  ...
  methods: {
    ...
    downloadDoc() {
      ...
      window.location.href = process.env.BASE_API + "/marketReport/getExcel3?reportId="+reportId+"&&feeId="+feeId;
    },
    //重置
    resetFrom(){
