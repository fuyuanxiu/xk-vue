        ...
        <el-col :span="24">
            <el-form :inline="true" :model="formQuery" class="demo-form-inline">
              <el-form-item label="物料号">
                <el-input v-model="formQuery.mateK3Code" placeholder="物料号"></el-input>
              </el-form-item>
...
import { getPriceChart } from '@/api/cost'
      ...
      getPriceChart(this.formQuery.mateK3Code,this.formQuery.startDate,this.formQuery.endDate,this.formQuery.flag).then(response => {
        ...
        if (response.result) {
          ...
          response.data.forEach(element => {
            //封装legend
            // this.legendData.push(this.formQuery.mateK3Code)
             lineChartData.legendData = this.formQuery.mateK3Code;
            //封装series
            seriesData.push(element.pcPrice);
              //封装xAxis
            this.xAxisData.push(element.pcMonth)
            
          });
          var sereisMap = {
              name: this.formQuery.mateK3Code,
              // smooth: true,//是否平滑展示
              type: 'line',
              data: seriesData,
