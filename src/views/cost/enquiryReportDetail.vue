...
export default {
  ...
  methods: {
    ...
    getBomDateByFileId(fileId) {
      ...
      getEqReport(filter).then(response => {
        if (!response.result) {
          this.$message.error(response.msg);
          return;
        }
    ...
    exportBom(){   
      ...
      process.env.BASE_API + "/report/getEqReportExcel?fileId=" + this.fileId;
    },
