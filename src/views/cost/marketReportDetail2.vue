...
import { getQtReport } from "@/api/marketReport";
import { getQtReportByBom } from "@/api/report";
import { getPermission } from '@/api/role';
export default {
  name: "marketReportDetail2",
  data() {
    return {     
      tableData: [],      
      tableHeader: [],
  ...
  methods: {
    ...
    toList() {
      this.$router.push({ path: "marketReportDetail", query: this.$route.query });
    },
    //选中行
    ...
    exportBom(){   
      ...
      process.env.BASE_API + "/marketReport/getQtReportExcel?fileId=" + this.fileId;
    },
    //同步
    updateReport(){
