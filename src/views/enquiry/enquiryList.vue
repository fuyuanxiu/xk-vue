        ...
        >
          <template slot-scope="scope">
            ...
            <MessageBox v-if="permit.DELETE" 
              ...
              confirmTitle="确认删除"
            ></MessageBox>
            <a v-if="permit.QUOTE_DETAIL"  class="operatIcon" title="报价明细" @click="showQuoDetail(scope.row)"><i class="el-icon-share"></i></a>
            <!-- <el-button title="报价明细" type="warning" icon="el-icon-more-outline" circle></el-button> -->
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
        ...
        if(response.result){
          if(response.data == "admin"){
            ...
            for(var i = 0; i < list.length; i++){
              ...
              if(list[i].permCode == "QUOTE_DETAIL") this.permit.QUOTE_DETAIL = true;
            }
          }
        }else{
    ...
    getData() {
      ...
      getEnquiryList(
        ...
        if (response.result) {
          ...
          this.items[1].num = response.data.draftNum;
          this.items[2].num = response.data.inNum;
          this.items[3].num = response.data.inNum2;
          this.items[4].num = response.data.completeNum;
          this.items[5].num = response.data.approvalNum;
        } else {
          this.$message.error(response.msg);
        }
      });
    ...
    showQuoDetail(row) {
      this.$router.push({path:'/quotation/quoMateDetail',query:{bsEqId:row.id}});
    },
  }
};
</script>
