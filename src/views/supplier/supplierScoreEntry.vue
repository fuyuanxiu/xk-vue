    ...
    <div style="padding:7px 25px 3px 22px">
        <el-card class="box-card" shadow="hover">
          <div slot="header" class="clearfix" >
            ...
            <el-button v-if="permit.SAVE" style="float: right;"  type="primary" size="mini" @click="Save()" >保存</el-button>
          </div>
      ...
      getPermByRouterCode(routerCode).then(response => {
        if(response.result){
          ...
          if(response.data == "admin"){
            this.permit.SAVE = true;
          }else{
            var list = response.data;
            for(var i = 0; i < list.length; i++){
              if(list[i].permCode == "SAVE") this.permit.SAVE = true;
            }
          }
        }else{
